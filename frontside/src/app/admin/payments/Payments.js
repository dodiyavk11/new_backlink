...
export class Payments extends Component {
  ...
  getAdminTransaction() {
    ApiServices.adminPaymetnTransaction().then(
      (res) => {
        if (res.data.status) {
          this.setState({
            rows: res.data.data,
  ...
  handleDownloadInvoice = (id, order_id, user_id) => {
    ...
    ApiServices.generateInvoicePdfAdmin(data).then(
      (res) => {
        if (res.status) {
          setTimeout(() => {
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        width: 50,
        sortable: false,
        renderCell: (row) => <span>{`${row.user.firstName} ${row.user.lastName}`}</span>,
      },
      {
        id: "email",
      ...
      },
      {
        ...
        sortable: true,
        renderCell: (row) => (
          <span>{CurrencyFormatter.formatCurrency(row.amount)}</span>
        ),
      },
      {
        id: "type",
        ...
        renderCell: (row) =>
          row.status !== "incomplete" ? (
            <span
              ...
              onClick={() =>
                this.handleDownloadInvoice(row.id, row.order_id, row.user_id)
              }
            >
              <i
