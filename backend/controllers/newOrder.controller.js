...
exports.textFileUpload = async(req, res) => {
  ...
  //       const addTempFile = await Models.tempFile.create({ fileName,fileKey,user_id });
		res.status(200).send({ status: true, message: "File upload successfully.", data: data });
	}
	catch(err)
	{
		console.log(err);
...
exports.addToCart = async(req, res) => {
	try{
		...
		{
			return res.status(422).send({ status:true, message: "Add to cart failed.", data: [] })
		}		
	}
	catch(err)
	{
...
exports.linkBundlePlaceOrder = async(req, res) => {
    try{
        ...
        const checkPlan = await Models.SubscriptionPlans.findOne({ where: { id:planId }});        
        ...
        if(userBalance)
        {
            ...
            return res.status(200).send({ status: true, message: "Your Order placed successfully" })
        }
        return res.status(400).send({ status: false, message: "Insufficient balance", error: "Insufficient balance" })
    }
    catch(err){
        res.status(500).send({ status: false, message: "Your order can not placed, an error occured", error: err.message })
    }
}